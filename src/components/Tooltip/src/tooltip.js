...
class Tooltip extends Component<Props, State> {
  ...
  getArrowDynamicStyle = () => {
    ...
    let left = anchorPoint.x - tooltipOrigin.x - ((width / 2) + marginLeft);
    return {
      ...(isRTL ? {right: left} : {left: left}),
    ...
    switch (this.state.placement) {
      case 'bottom':
        return {
          ...
          ...(isRTL ? {right: tooltipOrigin.x} : {left: tooltipOrigin.x})
        };
      case 'top':
        return {
